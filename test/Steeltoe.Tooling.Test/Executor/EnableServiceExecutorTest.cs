...
{
    public class EnableServiceExecutorTest : ToolingTest
    {
        [Fact]
        ...
        [Fact]
        public void TestEnableNonExistentService()
        {
            Assert.Throws<ServiceNotFoundException>(
                () => new EnableServiceExecutor("non-existent-service").Execute(Context)
            );
        }
    }
}
